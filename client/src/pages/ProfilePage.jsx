...
const ProfilePage = () => {
  ...
  const handleSubmit=async(e)=>{
     e.preventDefault()
     if(!selectedImage){
      await profileUpdate({fullName:name,bio})
      navigate('/')
      return ;
     }
     ...
     reader.onload=async()=>{
       ...
       await profileUpdate({profilePic:base64Image ,fullName: name, bio}) 
       navigate('/')
     }
  }
  ...
  return (
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        ...
        <img className={`max-w-44 aspect-square rounded-full mx-10 max-sm:mt-10 ${selectedImage && 'rounded-full'}`} src={authUser?.profilePic || '/Icon.svg'} alt="" />
      </div>
    </div>
  )
}
